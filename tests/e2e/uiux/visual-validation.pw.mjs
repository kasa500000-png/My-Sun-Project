...
const INITIAL_MEALS = [
  ...
  },
  {
    ...
    entryName: "저녁 식사",
    foods: [
      { id: "food-7", name: "연어 샐러드", portion: "1접시", calories: 420, carbs: 18, protein: 36, fat: 22 },
    ],
  },
];
...
async function installApiFixtures(page) {
  const state = {
    sessions: clone(INITIAL_SESSIONS),
    settings: clone(SETTINGS),
    meals: clone(INITIAL_MEALS),
    goal: clone(INITIAL_GOAL),
  };
...
async function capture(page, viewport, screen) {
  ...
  if (HARD_ASSERT) {
    ...
    const critical = (axe || []).filter(item => item.impact === "critical");
    expect(critical, `${screen.id} critical axe violations`).toEqual([]);
  }
...
const SCREENS = [
  {
    id: "AUTH-01",
    path: "/login",
    ready: page => expect(page.getByRole("heading", { name: "다시 만나 반가워요" })).toBeVisible(),
  },
  {
    id: "AUTH-02",
    path: "/login",
    prepare: page => page.getByRole("tab", { name: "회원가입 모드 선택" }).click(),
    ready: page => expect(page.getByRole("heading", { name: "기록을 시작해 볼까요?" })).toBeVisible(),
  },
  {
    id: "APP-01",
    path: "/uiux-visual?tab=home",
    ready: page => expect(page.getByRole("button", { name: /운동 기록하기/ }).first()).toBeVisible(),
  },
  {
    id: "APP-02",
    path: "/uiux-visual?tab=train",
    ready: page => expect(page.getByLabel("운동 검색")).toBeVisible(),
  },
  {
    id: "APP-03",
    path: "/uiux-visual?tab=log",
    ready: page => expect(page.getByRole("heading", { name: "기록 모아보기" })).toBeVisible(),
  },
  {
    id: "APP-04",
    path: "/uiux-visual?tab=balance",
    ready: page => expect(page.getByRole("heading", { name: "다음 운동을 정해볼까요?" })).toBeVisible(),
  },
  {
    id: "APP-05",
    path: "/uiux-visual?tab=diet",
    ready: page => expect(page.getByRole("heading", { name: "오늘 먹은 걸 남겨요" })).toBeVisible(),
  },
  {
    id: "APP-06",
    path: "/uiux-visual?tab=member",
    ready: page => expect(page.getByRole("heading", { name: "운동 설정" })).toBeVisible(),
  },
  {
    id: "STATE-01",
    path: "/uiux-visual/loading",
    ready: page => expect(page.getByText("기록을 준비하고 있어요")).toBeVisible(),
  },
  {
    id: "STATE-02",
    path: "/uiux-visual/error",
    ready: page => expect(page.getByRole("heading", { name: "기록을 불러오는 중 잠시 멈췄어요" })).toBeVisible(),
  },
  {
    id: "STATE-03",
    path: "/uiux-screen-that-does-not-exist",
    ready: page => expect(page.getByRole("heading", { name: "요청한 화면을 찾지 못했어요" })).toBeVisible(),
  },
  {
    id: "STATE-04",
    path: "/offline",
    ready: page => expect(page.getByRole("heading", { name: "연결이 돌아오면 안전하게 이어갈 수 있어요" })).toBeVisible(),
  },
];
  ...
  test.describe(viewport.id, () => {
    ...
    test(`capture ${MODE} visual matrix`, async ({ page }) => {
      ...
      for (const screen of SCREENS) {
        results.push(await capture(page, viewport, screen));
      }

      const reportDir = path.join(EVIDENCE_ROOT, "reports");
      await fs.mkdir(reportDir, { recursive: true });
      await fs.writeFile(
        path.join(reportDir, `${viewport.id}.json`),
...
test.describe("representative interactions", () => {
  ...
  test(`validate ${MODE} keyboard, form and modal flows`, async ({ page }) => {
    ...
    await search.fill("벤치프레스");
    await expect(page.getByRole("button", { name: /벤치프레스/ }).first()).toBeVisible();
    ...
    await fs.mkdir(reportDir, { recursive: true });
    await fs.writeFile(
      path.join(reportDir, "interactions.json"),
